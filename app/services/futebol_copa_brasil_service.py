...
class FutebolCopaBrasilService:
                        ...
                        for row in table:
                            if len(row) >= 14:
                                record = {
                                    ...
                                    "Estadio": row[9] if row[9] else None,
                                    "Cidade": row[10] if row[10] else None,
                                    "UF": row[11] if row[11] else None,
                                    "TV_1": row[12] if row[12] else None,
                                    "TV_2": row[13] if row[13] else None,
                                    "TV_3": row[14] if row[14] else None,
                                    "TV_4": row[15] if row[15] else None,
                                    "TV_5": row[16] if row[16] else None,
                                }
                                data.append(record)
            return data
        ...
        try:
            ...
            for record in data:
                ...
                """, (
                    ...
                    record["TV_3"], record["TV_4"], record["TV_5"]
                ))
            conn.close()
        except Exception as e:
    ...
    def get_all_texts(self, table_name, team_name: str = None, similarity_threshold: int = 80) -> List[Dict[str, str]]:
        ...
        try:
            ...
            for record in data:
                ...
                for tv_key in ["TV_1", "TV_2", "TV_3", "TV_4", "TV_5"]:
